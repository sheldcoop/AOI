# app.py
# Final Version: Reverted to the robust plotting loop.
...
PANEL_FILL_COLOR = '#F4A460'; PANEL_BORDER_COLOR = '#8B4513'
BG_COLOR = '#6B4226'; THIN_LINE_WIDTH = 2; THICK_LINE_WIDTH = 4
...
def generate_data():
    ...
    df = pd.DataFrame({'DEFECT_ID': list(range(1, NUM_DEFECTS + 1)), 'DEFECT_TYPE': np.random.choice(valid_defect_types, size=NUM_DEFECTS), 'UNIT_INDEX_X': np.random.randint(0, GRID_SIZE, size=NUM_DEFECTS), 'UNIT_INDEX_Y': np.random.randint(0, GRID_SIZE, size=NUM_DEFECTS)})
    ...
    if len(bottom_left_indices) > 5:
        ...
        df.loc[indices_to_change, 'DEFECT_TYPE'] = "System Error"
...
def transform_coords(df):
    plot_x = df['UNIT_INDEX_Y'] % PANEL_SIZE; plot_y = df['UNIT_INDEX_X'] % PANEL_SIZE
...
with col1:
    ...
    # --- THIS IS THE FIX: REVERTING TO THE ROBUST LOOP ---
    # We add a separate trace for each defect type, including our error type.
    # First, handle all known defect types.
    for dtype, color in DEFECT_STYLE_MAP.items():
        # Temporarily map "System Error" to our "Unknown" style for plotting
        if dtype == 'Unknown':
            dff = df[~df['DEFECT_TYPE'].isin(DEFECT_STYLE_MAP.keys())]
        else:
    ...
    fig.update_layout(width=800, height=800, plot_bgcolor=BG_COLOR, paper_bgcolor=BG_COLOR,
                      ...
                      shapes=shapes, margin=dict(l=0, r=0, t=0, b=0), showlegend=True,
