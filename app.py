# app.py
# FINAL VERSION: Built around the successful data loading strategy.
...
def load_and_clean_data(uploaded_file) -> Optional[pd.DataFrame]:
    try:
        # SUCCESSFUL STRATEGY: Read the file with no header, skipping the title row.
        df = pd.read_excel(uploaded_file, header=None, skiprows=1)

        # Keep only the first 6 columns which contain our data.
        df = df.iloc[:, :6]
        
        # Manually assign the correct column names based on the debug output.
        df.columns = [
...
if uploaded_file is None:
    ...
    if df is not None and not df.empty:
        ...
        with col1:
            ...
            fig.update_layout(plot_bgcolor='#F4A460', shapes=shapes, width=800, height=800,
