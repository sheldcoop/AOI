# app.py
# Final Definitive Version: Combines the most robust plotting method
# with the manual coordinate system for perfect control over layout and visuals.
...
DEFECT_STYLE_MAP = {
    ...
    'Unknown': 'red'  # Fallback for any defect type not in this list
}
...
def generate_data():
    ...
    # Intentionally add some errors to test the fallback color
    error_indices = np.random.choice(df.index, 5, replace=False)
    df.loc[error_indices, 'DEFECT_TYPE'] = "System Glitch" # This name is not in the style map
...
with col1:
    ...
    # Use the simple, robust single-trace method for plotting
    fig.add_trace(go.Scatter(
        x=df['plot_x'], y=df['plot_y'], mode='markers',
        marker=dict(color=df['color'], size=5, line=dict(width=0.5, color='black')),
        customdata=df.index, hoverinfo='none'
    ))

    # Manually draw panels and grids
    shapes = []
    ...
    # Use the manual coordinate system for perfect padding
